/* PeerJS networking for Poker - unlike shuffleboard's 1-on-1, the host can
   have several guests connected at once (one table, up to MAX_SEATS-1
   guests). Star topology: every guest only talks to the host; the host
   relays/broadcasts personalized state to each guest individually. */
(function (global) {
  ...
  const Net = {
    ...
    onData: null,          // (data, fromPeerId) => void
    onGuestConnected: null,  // (peerId) => void  (host side)
    onGuestLeft: null,       // (peerId) => void  (host side)
    onConnected: null,       // () => void        (guest side)
    onClose: null,           // () => void        (guest side, lost host)
    ...
    // host: send to one specific guest
    sendTo(peerId, obj) {
      const conn = this.conns.get(peerId);
      if (conn && conn.open) conn.send(obj);
    },

    // host: send to every connected guest (optionally skipping one)
